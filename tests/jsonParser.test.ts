...
describe("sanitizeJson (non-standard whitespace)", () => {
  it("parses JSON indented with non-breaking spaces", () => {
    const nbsp = "{\n\u00A0\u00A0\"name\": \"json-inspector\",\n\u00A0\u00A0\"v\": 1\n}";
    const r = parseJson(nbsp);
    expect(r.ok).toBe(true);
    if (r.ok) {
      const o = r.value as Record<string, unknown>;
      expect(Object.keys(o)).toEqual(["name", "v"]);
    }
  });

  it("preserves non-breaking spaces inside string values", () => {
    const r = parseJson('{"text": "a\u00A0b"}');
    expect(r.ok).toBe(true);
    if (r.ok) {
      const o = r.value as Record<string, unknown>;
      expect(o.text).toBe("a\u00A0b");
    }
  });
});
